...
class SensorAdapterManager(object):
	...
	This class serves as the central access point for interacting with one
	or more sensor adapters (e.g., temperature, humidity, pressure).
	It collects sensor readings, wraps them in SensorData objects, and
	notifies any registered data message listeners.

	Responsibilities:
	- Initialize and manage individual sensor adapter tasks.
	- Periodically poll or receive data from sensors.
	- Package sensor readings into SensorData objects.
	- Dispatch updates to DeviceDataManager or other listeners.
	- Handle lifecycle operations (start/stop) for all sensors.
	"""
	...
	def _initEnvironmentalSensorTasks(self):
		...
		if not self.useEmulator:
			# Simulator path (no Sense HAT required)
